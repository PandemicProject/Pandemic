...
inline void Clear()
{
	...
	for (int i = 0; i < 25; i++)
		mp[i].clear();
	IDall = 25;
	GANRAN = 0, GANRAN_now = 0, OUTT = 0, DEAD_now = 0;
}
bool flag = 0;
int GANRANlast;
inline void Change()
{ //每天更改各类参数
	...
	SDT2 = 3.6 / (700.0 * max(log2(FADay) / log2(1.1), 1.0) - 700 * ((rand() % 10) / 100.0));
	MED = min(pow(1.15, FADay - 10), 7.1);
}
struct data
{
	int x, y, Starve, disc, no, yes, die, hel, dec;
	bool last, vis;
} ps[MAXN + 1];
inline int opr() { return (rand() & 1) ? -1 : 1; }
inline void Infect(int i)
{ //i号致病
	ps[i].no = 2 + rand() % 5;
	++GANRAN;
...
inline void NewPerson(int i, int x, int y)
{
	ps[i].Starve = 0;
	ps[i].x = x;
	ps[i].y = y;
}
inline void Hospital(int i, bool nw)
{ //医院方面模拟
	if (nw)
	{
		flag = 1;
	...
	{
		if (!ps[i].die)
			ps[i].no = 0, ps[i].disc = 0, ps[i].hel = 1, ps[i].Starve = Day + 1, ps[i].vis = 0; //出院
		++BED;
		return;
	}
	return;
...
bool Gx[MAXN + 1];
inline double GetPi(double x, double y)
{ //人口分布
	x = x / del;
	y = y / del;
...
int Cen = 0;
inline void Push(int id, int i, int tim, bool v)
{
	if (!mp[tim][id])
		mp[tim][id] = ++IDall;
	Road[mp[tim][id]].push_back(i);
	Gx[mp[tim][id]] = v;
}
inline void GoOutside(int i)
{ //模拟出行函数
	...
	{ //附近出行
		...
		if (rand() % 100 < 60)
			if (ps[i].disc)
				x = opr() * rand() % Sq, y = opr() * rand() % Sq; //市区->郊区
			else
				x = opr() * rand() % 50, y = opr() * rand() % 50; //郊区->市区
		while (leng)
		{
			int d = rand() % 4;
...
{ //进行这一天的传播
	for (int i = 1; i <= IDall; i++)
	{
		...
		double no1 = (1.0 - (Gx[i] ? SDT2 : SpreadDelta)), noall = 1.0;
		int C = GET;
		GET = min(GET, rand() % 4 + 1);
		while (GET--)
			noall = noall * no1;
		noall = noall * 10000;
		int Inc = 0;
		for (int j = 0, sz = Road[i].size(); j < sz; j++)
		{
			if (rand() % 10000 + 1 <= noall || (ps[Road[i][j]].hel && rand() % 10 < 9))
				continue;
			else
				Infect(Road[i][j]), ++Inc;
		}
...
{ //建图
	...
	for (int i = 1; i <= top; i++)
	{
		...
		{
			++all;
			NewPerson(all, vc[i].a.first, vc[i].a.second);
		}
	}
	if (Sum2)
	{
		...
		{
			++all;
			NewPerson(all, opr() * (rand() % 50), opr() * (rand() % 50));
		}
	}
...
{ //输出模块
	...
	for (int i = 1; i <= all; i++)
	{
		if (ps[i].disc && (ps[i].disc != 2 || ps[i].yes || !ps[i].die))
			++GANRAN_now;
		if (ps[i].disc == 2 && ps[i].die && !ps[i].yes)
			++DEAD_now;
		if (ps[i].disc && !(ps[i].disc == 2 && ps[i].die && !ps[i].yes))
			s[BS + ps[i].y][BS + ps[i].x]++;
		if (ps[i].hel)
			++ZHIYU, ps[i].hel = 0;
		if (ps[i].disc)
			ps[i].last = 1;
	...
	for (int i = -Sq; i <= Sq; i++)
		for (int j = -Sq; j <= Sq; j++)
			s[BS + i][BS + j] += s[BS + i][BS + j - 1] + s[BS + i - 1][BS + j] - s[BS + i - 1][BS + j - 1];
